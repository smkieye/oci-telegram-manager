...
def _is_authed(request: Request) -> bool:
    ...
    return bool(settings.web_admin_password) and hmac.compare_digest(supplied, expected)
...
async def login_page(request: Request):
    return templates.TemplateResponse("login.html", {"request": request, "web_enabled": bool(settings.web_admin_password)})


@app.post("/login")
async def login(password: str = Form(...)):
    if not settings.web_admin_password or not hmac.compare_digest(password, settings.web_admin_password):
        return RedirectResponse("/login?error=1", status_code=303)
    response = RedirectResponse("/", status_code=303)
    response.set_cookie(COOKIE_NAME, _token(), httponly=True, samesite="lax", max_age=7 * 86400)
    return response
